  ...
  * @return mixed */
    ...
    public function __construct() {
        ...
        // ABSOLUTE & RELATIVE
            ...
            // 管理画面確認用にフルパスでのURLを許可するため、コメントアウト
    ...
    protected function checkFilePath($file) {
        ...
        //
        // 2019/9/5 [馬場] 特に問題なさそうなので チェック解除
        return true;
    }
    ...
    protected function checkUploadedFile(array $aFile=null) {
        ...
        elseif(!$this->checkFileSize($file['size'])) {
            return $this->label("ファイルの最大サイズに到達しました。");
        }
    ...
    protected function validateExtension($ext, $type) {
        ...
        // 許可リストへ変更
        $exts = strtolower(text::clearWhitespaces($this->config['allowedExts']));
        if (strlen($exts)) {
            $exts = explode(" ", $exts);
            // 許可リストに無い拡張子の場合は拒否
    ...
    protected function checkFileSize($size) {
        if(!isset($this->config['uploadMaxsize'])) {
            // maxサイズが設定されていない場合は
            return true;
        }
        $maxsize = 0;
        if(preg_match('/^\d+$/',$this->config['uploadMaxsize'])) {
            $maxsize = $this->config['uploadMaxsize'];
        } else if(preg_match('/^(\d+)B$/i',$this->config['uploadMaxsize'],$matches)) {
            $maxsize = $matches[1];
        } else if(preg_match('/^(\d+)KB$/i',$this->config['uploadMaxsize'],$matches)) {
            $maxsize = $matches[1] * 1024;
        } else if(preg_match('/^(\d+)MB$/i',$this->config['uploadMaxsize'],$matches)) {
            $maxsize = $matches[1] * 1024 * 1024;
        } else if(preg_match('/^(\d+)GB$/i',$this->config['uploadMaxsize'],$matches)) {
    ...
    // トータルのファイルサイズチェック
    protected function checkTotalFileSize() {
        if(!isset($this->config['totalUploadMaxsize'])) {
            // maxサイズが設定されていない場合は
            return true;
        }
        $maxsize = 0;
        if(preg_match('/^\d+$/',$this->config['totalUploadMaxsize'])) {
            $maxsize = $this->config['totalUploadMaxsize'];
        } else if(preg_match('/^(\d+)B$/i',$this->config['totalUploadMaxsize'],$matches)) {
            $maxsize = $matches[1];
        } else if(preg_match('/^(\d+)KB$/i',$this->config['totalUploadMaxsize'],$matches)) {
            $maxsize = $matches[1] * 1024;
        } else if(preg_match('/^(\d+)MB$/i',$this->config['totalUploadMaxsize'],$matches)) {
            $maxsize = $matches[1] * 1024 * 1024;
        } else if(preg_match('/^(\d+)GB$/i',$this->config['totalUploadMaxsize'],$matches)) {
            $maxsize = $matches[1] * 1024 * 1024 * 1024;
        } else {
            return false;
        }
        ...
        if(is_array($this->file['size'])){
            foreach($this->file['size'] as $byte) {
                $Used += $byte;
            }
        }
        if($maxsize < $Used) {
    ...
    // ディレクトリのトータルのアップロードファイルサイズチェック
    protected function checkDirMaxSize($dir) {
        if(!isset($this->config['dirMaxsize'])) {
            // maxサイズが設定されていない場合は
            return true;
        }
        $maxsize = 0;
        if(preg_match('/^\d+$/',$this->config['dirMaxsize'])) {
            $maxsize = $this->config['dirMaxsize'];
        } else if(preg_match('/^(\d+)B$/i',$this->config['dirMaxsize'],$matches)) {
            $maxsize = $matches[1];
        } else if(preg_match('/^(\d+)KB$/i',$this->config['dirMaxsize'],$matches)) {
            $maxsize = $matches[1] * 1024;
        } else if(preg_match('/^(\d+)MB$/i',$this->config['dirMaxsize'],$matches)) {
            $maxsize = $matches[1] * 1024 * 1024;
        } else if(preg_match('/^(\d+)GB$/i',$this->config['dirMaxsize'],$matches)) {
            $maxsize = $matches[1] * 1024 * 1024 * 1024;
        } else {
            return false;
        }
