...
class Action extends AuthAction {
	...
	function execute() {
		...
		/* author type list */
		$result = $db->statement('admin/publisher/author/sql/author_type_list.sql');
		...
		if( $this->search_submit || 1){
			$result = $db->statement('admin/publisher/author/sql/list.sql');

			$tree = $db->buildTree($result, 'author_no');
			$this->authorList = $tree;

			//facility
			$result = $db->statement('admin/publisher/author/sql/list_total.sql');
