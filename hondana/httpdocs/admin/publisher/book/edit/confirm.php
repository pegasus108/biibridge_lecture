...
class Action extends AuthAction {
	...
	function prepare() {
		...
		if(empty($_REQUEST["livOld"])){
			...
			if(!isset($_REQUEST['clear_actibook'])){
				$this->clear_actibook = NULL;
			}
		}
		...
		if(isset($this->news_relate_list)) {
			...
			$this->recommend_count = $db->statementFetch("other_recommend_count");
			$this->recommend_count = $this->recommend_count['recommend_count'];
		}
	...
	function validate() {
		...
		if (!$this->isbn) {
			...
			$result = $db->query("select * from book where isbn='{$this->isbn}' and book_no<>{$this->book_no} and sync_allowed is not null limit 1;");
...
//			$this->errors['aaa'] = '著者名カナの選択をご確認ください。';
		...
		if($this->recommend_status=='1' && $this->publisher['recommend_type'] == 2) {
			if (!(isset($this->recommend_image['http_path']) || isset($this->old_recommend_image))) {
...
//			$this->errors['aaa'] = '著者名カナの選択をご確認ください。';
		...
		if (!$this->{$key}) {
		} elseif( !mb_ereg_match('^.{0,1300}$', preg_replace('/(\r\n|\r|\n)/', '<br>',$this->{$key})) ){
			$this->errors[$key] = '内容紹介2は1300文字以内でご記入ください。(改行は<br>に置き換えられます。)';
		} elseif($this->jpoSync && preg_match('/<(?!p>|P>|br|BR|p |P |\/p>|\/P>)/', $this->{$key})) {
			$this->errors[$key] = '内容紹介2には<br>タグと<p>タグのみを利用可能です。';
		}
