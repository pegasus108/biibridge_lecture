...
class Action extends AuthAction {
	function execute() {
		...
		$result = $db->statement('admin/publisher/book/label/sql/check.sql');
		$tree = $db->buildTree($result);
		if (!empty($tree[0]['rgt']) && !empty($tree[0]['lft']) && !empty($tree[0]['myDistance'])) {
			if ($this->order == "up") {
				$db->assign('myL', $tree[0]['lft']);
				$result = $db->statement('admin/publisher/book/label/sql/check_up.sql');
			}elseif($this->order == "down"){
				$db->assign('myR', $tree[0]['rgt']);
				echo $tree[0]['rgt'];
				$result = $db->statement('admin/publisher/book/label/sql/check_down.sql');
			} else {
		...
		if(!count($errors)) {
			$db->begin();
			if($this->order == "up"){
				$db->statement('admin/publisher/book/label/sql/sort_up.sql');
			}elseif($this->order == "down"){
				$db->statement('admin/publisher/book/label/sql/sort_down.sql');
			}
			$db->commit();
		} else {
