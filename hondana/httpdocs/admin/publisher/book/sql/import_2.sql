	...
	values
		{foreach name=genre from=$book[29] item=genre}
			{if !$smarty.foreach.genre.first},{/if}(
				...
				@myNo+{$smarty.foreach.book.iteration},
				(select ifnull(genre_no,0) from genre where publisher_no = {$publisher_no} and genre.name = BINARY '{$genre}' limit 1),
				current_timestamp
			)
		{/foreach}
	;
	...
	values
		{foreach name=series from=$book[30] item=series}
			{if !$smarty.foreach.series.first},{/if}(
				...
				@myNo+{$smarty.foreach.book.iteration},
				(select ifnull(series_no,0) from series where publisher_no = {$publisher_no} and series.name = BINARY '{$series}' limit 1),
				current_timestamp
			)
		{/foreach}
	;
