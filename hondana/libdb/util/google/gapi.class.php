...
class gapi
{
  ...
  protected function processFilter($filter)
  {
    ...
    $filter = preg_replace('/\s\s+/u',' ',trim($filter)); //Clean duplicate whitespace
    $filter = str_replace(array(',',';'),array('\,','\;'),$filter); //Escape Google Analytics reserved characters
    $filter = preg_replace('/(&&\s*|\|\|\s*|^)([a-z]+)(\s*' . $valid_operators . ')/ui','$1ga:$2$3',$filter); //Prefix ga: to metrics and dimensions
    $filter = preg_replace('/[\'\"]/ui','',$filter); //Clear invalid quote characters
    $filter = preg_replace(array('/\s*&&\s*/u','/\s*\|\|\s*/u','/\s*' . $valid_operators . '\s*/u'),array(';',',','$1'),$filter); //Clean up operators
  ...
  protected function reportObjectMapper($xml_string)
  {
    ...
    foreach($google_results->aggregates->metric as $aggregate_metric)
    {
      ...
      if(preg_match('/^(\d+\.\d+)|(\d+E\d+)|(\d+.\d+E\d+)$/u',$metric_value))
      {
  ...
  {
    if(!preg_match('/^get/u',$name))
    {
      throw new Exception('No such function "' . $name . '"');
    }
    
    $name = preg_replace('/^get/u','',$name);
